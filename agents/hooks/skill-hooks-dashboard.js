...
function buildReport(result, args) {
  ...
  const summaryLines = [
    ...
    'node %USERPROFILE%\\.agents\\hooks\\skill-hooks-dashboard.js --format json --out %USERPROFILE%\\.agents\\logs\\skill-hooks-weekly.json',
    'node %USERPROFILE%\\.agents\\hooks\\skill-hooks-dashboard.js --days 14',
    'node %USERPROFILE%\\.agents\\hooks\\skill-hooks-dashboard.js --days 7 --out %USERPROFILE%\\.agents\\logs\\skill-hooks-weekly.md',
    '```',
    '',
    '判定说明：',
    '- `L0`：入口路由',
