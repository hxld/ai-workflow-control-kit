...
13. Planning Brainstorm Gate：对非平凡需求执行自我苏格拉底式方案盘问，输出为什么这样做、为什么不那样做、隐藏 surface 和验证断言。
14. 对新增/重构模块执行 Architecture Depth Gate，确认接口是否够小、测试面是否正确、seam 是否真实。
15. 生成 Expected Diff Matrix；若复用既有实现，先拆 `intended_change_slice` 与 `out_of_scope_drift`。
16. 生成 Test Design Control：影响范围、风险分级、判定表、可执行步骤、覆盖校验；小需求可用 mini form。
17. 大需求先生成 Capability Slice Matrix，再结构化实现单元。
18. 用户要求自主实现或 90%+ 覆盖时，生成 90% Coverage Plan，明确必须实现、可后置和必须暂停确认的项。
19. 对跨边界、不可逆、高返工或高置信但不可由编译器证明的决策，生成 Decision Doubt Checkpoint；命中决策记录条件时写 Decision Record Candidate。
20. 设计问题不清但可低成本实证时，生成 Throwaway Prototype Plan，并定义删除或吸收条件。
21. 同步产出 `.doc/<feature>/tech-design.md` 与 `openspec/changes/<change>/...`。
22. 进入用户审批门。
...
```markdown
| claim | why it matters | artifact/contract | doubt questions | reconcile result |
|-------|----------------|-------------------|-----------------|------------------|
```
...
默认输出 `状态 / 范围内外 / 领域语言差异 / 冻结表 / Planning Brainstorm Matrix / Surface 矩阵 / Expected Diff Matrix / Test Design Control / 90% Coverage Plan / Verification Baseline Plan / Architecture Depth Gate / Decision Doubt Checkpoint / Decision Record Candidate / Throwaway Prototype Plan / 实现单元 / 测试策略 / OpenSpec / 人审问题`。长模板见 `references/plan-template.md`。

## 完成标准

进入 `dev-workflow` 前必须满足：阻断问题清零、冻结表完整、Planning Brainstorm Matrix 带稳定 PB-ID 且完整或有合法 `skip_reason`、多 surface 有验证点、Expected Diff Matrix 完整、Test Design Control 完整或有合法 mini/skip reason、capability slices/90% Coverage Plan/Verification Baseline Plan 完整、`.doc` 与 OpenSpec 落盘且用户审批通过。否则输出 `Implementation Readiness = NO-GO`，按 delivery kit 补齐缺口。
