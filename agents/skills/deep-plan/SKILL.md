...
**下游技能：** `dev-workflow`, `gen-tests`, `backend-effort-estimate`
...
没有冻结表、Surface 矩阵、必要的 Expected Diff Matrix 和风险缩放后的 Test Design Control，不得把规划标记完成。
...
3. Branch/Commit Reconstruction：若无 PRD，保留反推需求置信度、Diff Role Matrix 和 verification gap。
...
8. Verification Baseline Plan：定义基线探测、RED/GREEN 命令和 blocker 分类。
9. 研究现有实现：入口、服务、mapper、DTO、状态、日志、测试。
10. Surface Mining Pass：把真实入口、编排服务、副作用、生成物、前端/导出和测试承载面挖出来。
11. Change Impact Search：对规则、字段、枚举、数据来源、方法签名和 shared helper 做只读影响搜索。
12. 审查接口契约、对标实现、可观测性、显式需求契约。
13. 若上游有 Same Symptom Branch Matrix，生成 Branch Coverage Plan，说明覆盖、排除、验证和 blocker。
14. Planning Brainstorm Gate：对非平凡需求执行自我苏格拉底式方案盘问，输出为什么这样做、为什么不那样做、隐藏 surface 和验证断言。
15. 对新增/重构模块执行 Architecture Depth Gate，确认接口是否够小、测试面是否正确、seam 是否真实。
16. 生成 Expected Diff Matrix；若复用既有实现，先拆 `intended_change_slice` 与 `out_of_scope_drift`。
17. 生成 Test Design Control：影响范围、风险分级、判定表、可执行步骤、覆盖校验；小需求可用 mini form。
18. 大需求先生成 Capability Slice Matrix，再结构化实现单元。
19. 用户要求自主实现或 90%+ 覆盖时，生成 90% Coverage Plan，明确必须实现、可后置和必须暂停确认的项。
20. 对跨边界、不可逆、高返工或高置信但不可由编译器证明的决策，生成 Decision Doubt Checkpoint；命中决策记录条件时写 Decision Record Candidate。
21. 设计问题不清但可低成本实证时，生成 Throwaway Prototype Plan，并定义删除或吸收条件。
22. 同步产出 `.doc/<feature>/tech-design.md` 与 `openspec/changes/<change>/...`。
23. 进入用户审批门。

## 必须冻结

### Domain Fact Sheet Gate（硬门禁）
...
```markdown
| inferred requirement | confidence | evidence | missing source | verification gap | plan action |
|----------------------|------------|----------|----------------|------------------|-------------|
| file family | role | expected/surprising | shared impact | validation |
```
...
```markdown
| symptom branch | precondition / entry | plan action | expected file family | validation / must-not | status |
|----------------|----------------------|-------------|----------------------|-----------------------|--------|
```
...
### Surface Mining Pass（硬门禁）

多 surface、大需求或同时出现自动流转、报表/导出、前端、日志、任务、图片/附件/模板、外部协议时，Expected Diff Matrix 之前必须先做只读挖掘：

```markdown
| capability | runtime entry | orchestration service | side effects/tables | generated artifacts/templates | frontend/export surface | tests/harness | evidence | gap |
|------------|---------------|-----------------------|---------------------|-------------------------------|-------------------------|---------------|----------|-----|
```
...
### Change Impact Search & Consistency Matrix
...
```markdown
| 需求项 | Scope 分类 | 预计模块 | 预计文件族 | 预计新增/修改类型 | 不应触碰文件族 | 验证方式 | 闭环条件 |
|--------|-----------|----------|------------|-------------------|----------------|----------|----------|
```
...
```markdown
| Step | Output | Human decision | AI assistance | Status |
|------|--------|----------------|---------------|--------|
| Impact scope | affected interfaces/pages/tasks/surfaces | final test scope | code/diff/entry scan | |
| Risk grading | risk and depth per target | risk level and depth | change/branch/old-data analysis | |
| Decision table | minimal case combinations | keep/merge business cases | expand and merge combinations | |
| Executable steps | fixture/action/assertions | special scenarios and blockers | step generation | |
| Coverage check | surface x dimension status | accept gaps/blockers | coverage matrix | |
```
...
### Capability Slice Matrix

跨模块、多 surface、从 0 新增模块、外部集成或大量文件族时，不得只生成一个巨型计划，必须先切能力包：

```markdown
| slice | requirement rows | surfaces | data/model | expected file families | tests | DoD | dependencies |
|-------|------------------|----------|------------|------------------------|-------|-----|--------------|
```
...
```markdown
| slice | weight | core/supporting/optional | first implementation target | behavior test charter | stop condition |
|-------|--------|--------------------------|-----------------------------|-----------------------|----------------|
```
...
```markdown
| slice | priority | coverage weight | must implement for 90% | defer / ask | DoD | verification | blockers |
|-------|----------|-----------------|------------------------|-------------|-----|--------------|----------|
```
...
### Decision Doubt Checkpoint

非平凡决策在进入实现前必须写成可反驳的小块：

```markdown
| assertion | why it matters | artifact/contract | doubt questions | reconcile result |
|-------|----------------|-------------------|-----------------|------------------|
```
...
### Architecture / Prototype / Decision Gates
...
默认输出 `状态 / 范围内外 / 领域语言差异 / 冻结表 / Branch Coverage Plan / Planning Brainstorm Matrix / Surface 矩阵 / Expected Diff Matrix / Test Design Control / 90% Coverage Plan / Verification Baseline Plan / Architecture Depth Gate / Decision Doubt Checkpoint / Decision Record Candidate / Throwaway Prototype Plan / Subagent Evidence Ledger / 实现单元 / 测试策略 / OpenSpec / 人审问题`。长模板见 `references/plan-template.md`。

## 完成标准

进入 `dev-workflow` 前必须满足：阻断问题清零、冻结表完整、L2/L3 或上游同症状矩阵已形成 Branch Coverage Plan、Planning Brainstorm Matrix 带稳定 PB-ID 且完整或有合法 `skip_reason`、多 surface 有验证点、Expected Diff Matrix 完整、Test Design Control 完整或有合法 mini/skip reason、capability slices/90% Coverage Plan/Verification Baseline Plan 完整、`.doc` 与 OpenSpec 落盘且用户审批通过。否则输出 `Implementation Readiness = NO-GO`，按 delivery kit 补齐缺口。
