...
触发本技能后必须按顺序推进。每个阶段先产出对应交付物，满足门禁后再进入下一阶段。

```
[0] ⚠️ REQUIRED 预检查 + 模式确认 → 输出：模式/范围/契约/验证/退出/同步 + 任务清单 + 依赖图；命中多落点风险时先过需求意图对齐门
[1] ⚠️ REQUIRED 需求分析 → 输出：隐藏需求清单（via req-alignment-check）
[2] ⚠️ REQUIRED 技术设计 → 输出：**同时生成** `openspec` 变更 + `.doc/` 设计 + 测试设计控制
[2.5] ⚠️ RISK-SCALED 实现前方案审查 → L2/L3、review pressure、多 surface 或高返工成本时调用 `deep-review` 的方案审查模式；P0/P1 必须回到 `deep-plan`
...
[5] ⚠️ REQUIRED 测试 → 输出：测试报告（via gen-tests，必须消费测试设计控制，并写明验证范围、失败阶段复验、反向断言与覆盖矩阵状态）
[6] (conditional) 注释 → 输出：注释代码（via add-comments）
[7] ⚠️ REQUIRED 验证 + `sync-progress` 收口 → 输出：交接摘要 + `.doc` / `openspec` / `.memory` 三路同步账本
```
...
- [ ] 已冻结 `模式 / 范围 / 契约 / 验证 / 退出 / 同步`，且“退出”写明何时回到需求/设计、何时停止修复。
...
- [ ] `req-alignment-check` 和 `deep-plan` 已完成；命中 L2/L3、review pressure、多 surface 或高返工成本时，已完成实现前方案审查。
- [ ] 技术设计已批准（Phase 3 通过）。
...
- [ ] 非平凡、多 surface、状态流转、外部接口、报表/导出、前端可见面、异步或 must-not 需求已生成测试设计控制；小需求有合法 mini form 或 skip reason。
- [ ] 若目标是自主实现或 90%+ 覆盖，已存在需求覆盖账本 / 90% 覆盖计划，并明确 `core_path`、权重、可后置项和暂停确认项。
...
| 把内部排障诉求落到用户/业务可见 helper | 先过需求意图对齐门（Intent Alignment Gate），拆清可见契约与内部可观测面 |
...
| 小测试 GREEN 就宣布完成 | 用完成门核对冻结表、surface 矩阵、预计变更矩阵 |
| 测试设计控制只停在文档 | Phase 5 必须让 `gen-tests` 消费测试设计控制，并在收口写覆盖矩阵状态 |
| 技术方案没先审就开写 | L2/L3、review pressure、多 surface 或高返工成本时，先走实现前方案审查；P0/P1 未清零不得进入 Phase 4 |
| 用户纠错后只修当前点 | 回退 `req-alignment-check`，执行 User Correction Escalation Gate，同类字段/入口/错误提示/数据源/文案一起扫 |
| 把局部编译或单测通过当作需求完成 | 收口前必须有完成证据账本、最终完整性门和真实 surface 证据；缺失只能 `PARTIAL` |
...
- Phase 2.5 实现前方案审查按风险触发；L2/L3、review pressure、多 surface、自主实现 90%+ 或高返工成本任务缺审查时，不得进入 Phase 4 或完成态。
...
- 实际 diff 必须与预计变更矩阵对齐；未预测文件、缺失预测文件、跨域文件或 scope drift 都要回退 `deep-plan`。
...
- 没有完成证据账本、最终完整性门、真实 surface 证据和剩余风险披露时，禁止使用“完成 / 全部完成 / 已按需求完成 / 可以提交”这类完成态表述；只能说明已验证项、未验证项和 blocker。
...
- 用户可见产物中文优先：`.doc/<feature>/tech-design.md`、`task_plan.md`、`code-change.md`、测试报告、审查报告默认使用中文标题和中文表头；英文术语只放括号或作为内部状态码。
...
用户可见标题、章节名和矩阵表头使用中文优先，例如“需求意图对齐门”“规划盘问矩阵”“预计变更矩阵”“测试设计控制”“实现前方案审查结果”；英文名称只作为括号别名。
```
...
## Phase 2.5: 实现前方案审查（按风险触发）

触发条件：`review_tier=L2/L3`、review pressure、多 surface、外部接口、状态流转、落库、异步、报表导出、自主实现 90%+、用户已纠错或高返工成本。

审查对象：

- 需求冻结表、字段/数据来源、Surface 覆盖矩阵。
- 规划盘问矩阵、预计变更矩阵、测试设计控制、能力切片和 90% 覆盖计划。
- 决策反证点、开放假设、阻断项和用户审批问题。

执行规则：

- 调用 `deep-review` 的 `PRE-IMPLEMENTATION-REVIEW` 模式；默认只读审查。
- P0/P1 发现必须回到 `deep-plan` 修正方案和 `.doc` / `openspec`，不能直接进入实现。
- 小型机械改动可跳过，但必须写合法 `skip_reason`。
- 审查结论写回 `tech-design.md` 的“实现前方案审查结果”章节，供 `sync-progress` 收口核对。
...
| 5 | 测试 + 修复循环（最多3次），消费测试设计控制并输出覆盖矩阵状态 | `gen-tests` |
| 6 | 添加注释 | `add-comments` |
...
先调用 `sync-progress` 做最终收口，再输出交接摘要。
