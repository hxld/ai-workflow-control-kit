...
## Kibana Discover URL Backend Query
...
{"index":"{resolvedIndexPattern}","ignore_unavailable":true}
...
Rule: if Kibana backend endpoints are reachable, do not ask the user to manually search Kibana just because `ES_PROD_URL` is empty.
