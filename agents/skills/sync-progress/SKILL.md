...
任何完成态声明都必须能被完成证据账本和最终完整性门反查。若本轮出现用户纠错、返工或“之前说完成但实际未完成”的信号，还必须给出纠错闭环账本；缺失时不得提交、推送或宣称完成。
用户可见收口文档中文优先：`code-change.md`、进度同步报告、审查账本和矩阵表头默认使用中文；英文术语只作为括号别名或内部状态码。
...
| Mode | 收口重点 |
|------|----------|
| `planned-dev` | 完成证据账本、覆盖账本、文档/规格同步、完整验证、最终完整性门、提交边界 |
...
只有 `planned-dev`、`hotfix` 和 `tooling` 需要完成证据账本或等效完成证据。

## 审查等级收口账本（Review Tier Closure Ledger）

收口前必须消费 `workflow-router` 或当前任务事实推导出的 `review_tier`。缺少显式 tier 时按风险补判，不能用未声明规避审查。

```markdown
| 审查等级 | 触发证据 | 已执行审查镜头 | 已检查反证 | 同症状分支 | 未解决缺口 | 最终上限 |
|----------|----------|----------------|------------|------------|------------|----------|
```
...
- `L2`：生产结论、热修、修数、缓存、外部接口或跨组件归因必须至少有证据链、同症状分支矩阵或反证检查；计划进入实现前应有方案审查或明确 `skip_reason`，缺失时最终状态最高 `PARTIAL`。
- `L3`：发布、批量数据影响、资金/状态推进、用户纠错后继续、曾提前完成或多 surface review pressure，必须有实现前方案审查 + 提交前 `deep-review` 多镜头或等效审查账本；缺失时不得提交、推送或宣称完成。
- `same-symptom branches` 不能只写“无”；必须说明查过哪些入口、配置、缓存、异步、下游或展示分支，或写 `not_applicable:<reason>`。

## 实现前方案审查收口

`planned-dev` 命中 L2/L3、review pressure、多 surface、自主实现 90%+ 或高返工成本时，收口必须核对：

```markdown
| 审查项 | 方案位置 | 审查结论 | P0/P1 是否清零 | 剩余风险 | 状态影响 |
|--------|----------|----------|----------------|----------|----------|
```

规则：

- 未执行且无合法 `skip_reason`：最终状态最高 `PARTIAL`。
- 有 P0/P1 未清零：状态为 `BLOCKED`，不得提交、推送或宣称完成。
- 审查发现导致方案变化时，必须同步 `.doc/<feature>/tech-design.md` 和 OpenSpec。
...
## 子 agent 接收账本（Subagent Intake Ledger）

`sync-progress` 只消费子 agent 结果，不负责派遣。若本轮使用过子 agent，收口必须把结果并入主会话证据账本：

```markdown
| 子 agent 结果 | 采纳项 | 拒绝项 | 主会话复核 | 最终状态影响 |
|---------------|--------|--------|------------|--------------|
```
...
- 生成物账本（Generated Artifact Ledger）：`路径 -> 来源 -> 生成物/有效变更 -> 保留/删除 -> 提交/不提交`
- 原型、临时 harness、调试脚本是否已删除、吸收或标记为 generated/debug artifact
- Scope 分类：范围内、前端-only、已有需证据、待确认、支撑工具、无关漂移
- 阶段状态账本（Phase State Ledger）：长任务或多 slice 时，收集 `.doc/<feature>/phase-state.md`、已有 `.artifacts/phase-state.md` 或任务文档中的 phase checkpoint；对比 final diff/test 状态，缺失或不一致时标 `resume_state_gap`，整体最高 `PARTIAL`

## 预计变更闭环账本（Expected Diff Closure Ledger）

大需求、自主实现、replay/eval 或多 surface 收口时，必须把计划中的预计变更矩阵转成闭环账本：

```markdown
| 预计文件族 | 需求/切片 | 实际 diff 状态 | 验证方式 | 闭环状态 | 覆盖率影响 |
|------------|-----------|------------------|----------|----------|------------|
```
...
## 测试设计闭环账本（Test Design Closure Ledger）

非平凡 `planned-dev`、多 surface、状态流转、外部接口、报表/导出、前端可见面、异步或 must-not 需求收口时，必须把 `deep-plan` 产出的测试设计控制转成测试闭环账本：

```markdown
| 测试设计行 | 目标/surface | 计划风险/深度 | 已执行证据 | 覆盖状态 | 缺口/行动 |
|------------|----------------|----------------|------------|----------|-----------|
```

`coverage status` 只能是：`covered`、`partial`、`blocked:<reason>`、`not_applicable:<reason>`。

如果 `gen-tests` 没有消费测试设计控制，或覆盖矩阵缺少高风险 surface / must-not / 副作用行，整体状态最多 `PARTIAL`，不得进入“完成 / 可提交”表述。

## Diff 角色矩阵（Diff Role Matrix）

大需求、历史对比、replay/oracle 对比或 final diff 收口时，实际 diff 必须按角色分类：

```markdown
| 文件族 | 角色 | 原因 | 验证方式 | 是否提交/纳入 |
|--------|------|------|----------|--------------|
```
...
|--------|---------------|--------|
| `.doc/<feature>/tech-design.md` | 设计、接口契约、字段来源、风险或实现边界变化 | 回写最终决策、测试设计控制消费结果、验证计划和偏差原因 |
| `.doc/<feature>/task_plan.md` | 任务状态、剩余项、阻塞项变化 | 勾选或更新任务状态，不保留过期 TODO |
| `.doc/<feature>/code-change.md` | 有代码、测试、SQL、配置或交付文档有效变更 | 写“最终变更说明”：最终范围、关键代码/SQL、验证结果、未覆盖项 |
...
`code-change.md` 默认写中文“最终变更说明”，不默认展开 commit-by-commit。若 `.doc/`、`openspec/` 或 `.memory/` 被 ignore，仍按本地真值同步，但不得默认 `git add -f`。
...
```markdown
| 需求/切片 | 来源引用 | 代码/文档位置 | 验证命令/结果 | 损耗/披露 | 状态 |
|-----------|----------|----------------|----------------|-----------|------|
```

`status=DONE` 不能只依赖局部编译、局部单测或“代码已改”。涉及 UI、表单、下拉、错误提示、接口出参、数据来源、落库、导出或异步副作用时，完成证据账本必须包含可见 surface 证据；未跑通真实入口时，在 `lossy/disclosure` 写明 `not_run`、原因和剩余风险。
...
## 规划盘问追踪账本（Planning Brainstorm Trace Ledger）

非平凡 `planned-dev`、review pressure、自主实现、用户已纠偏、多 surface 或高返工成本任一命中时，收口必须反查 `deep-plan` / `tech-design.md` 中的规划盘问矩阵：

```markdown
| PB 编号 | 已选决策 | 放弃方案 | 实现证据 | 验证断言 | 最终状态 |
|---------|----------|----------|----------|----------|----------|
```
...
- 规划盘问矩阵缺失且没有合法 `skip_reason` 时，整体 `Implementation Readiness` 不能从收口阶段补成 `DONE`；必须回到 `deep-plan`。
...
```markdown
| 决策/范围项 | 存放位置 | 来源引用 | 置信度 | 最近验证 | 未来触发 | 状态 |
|-------------|----------|----------|--------|----------|----------|------|
```
...
## 加权覆盖账本（Weighted Coverage Ledger）

当本轮目标包含“自主实现 / 少问 / 覆盖 90%+”时，收口报告必须用覆盖账本声明是否达标：

```markdown
| 需求/切片 | 优先级 | 权重 | 实现状态 | 验证方式 | 计入覆盖 | 缺口原因 |
|-----------|--------|------|----------|----------|----------|----------|
```
...
## Phase 3.5: 最终完整性门（Final Completeness Gate）
...
| 测试设计控制 | 影响范围、风险分级、判定表、可执行步骤、覆盖校验已被测试生成消费并闭环 |
| 预计变更矩阵 | 实际 diff 未缺预测文件，未混入未解释文件，且每个预计文件族有闭环状态 |
| TDD 证据 | RED/GREEN 命令、失败阶段、逐行 RED 证据、复验结果完整 |
| 规划盘问追踪 | 非平凡需求有带 PB-ID 的规划盘问矩阵或合法 `skip_reason`，关键取舍已落到实现证据和验证断言 |
...
| 实现前方案审查 | L2/L3、review pressure、多 surface 或高返工成本任务已在开发前审方案，P0/P1 清零或有合法跳过理由 |
| 审查等级收口 | L2/L3 风险任务有审查镜头、反证检查、同症状分支和 unresolved gap；缺失则最高 `PARTIAL` |
| Review Independence Disclosure | 多轮/第二视角/隔离线程/外部 reviewer 已披露 review mode、independence level、外部模型使用和主会话复核 |
| 完成态表述门禁 | “完成 / 全部完成 / 已按需求完成 / 可以提交”有完成证据账本支撑；否则只能写 `PARTIAL` 或 blocker |
...
branch/commit-derived 报告必须附反推需求矩阵、Diff 角色矩阵和验证缺口；compile green 只能算结构验证。
