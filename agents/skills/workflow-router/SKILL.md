...
- 如果任务仍在实现链路中，保持原技能链，但把 `CONTRACT` 标为 `review_pressure`，`VERIFY` 必须包含证据来源、确定性、验证方式和无法验证的假设；在技术设计后、实现前触发“实现前方案审查”。
...
## 子 agent 适用性门（Subagent Eligibility Gate）
...
## 实现前方案审查门（Plan Review Gate）

把审查前移到技术设计之后、写代码之前。目标是先审“做什么、为什么这样做、哪里会漏、怎么证明”，而不是等代码写完才发现方案错。

触发信号：

- `review_tier=L2/L3`、review pressure、发布/提交前高风险、用户已纠错、曾提前完成。
- large / 多 surface / 外部接口 / 状态流转 / 落库 / 异步 / 报表导出 / 自主实现 90%+。
- 技术方案里有 `Decision Doubt Checkpoint`、高权重 `core_path`、高权重 deploy-facing surface，或 Expected Diff / Test Design 存在假设行。

路由规则：

- `quick/small` 且无上述信号：可跳过，写 `skip_reason=no_requirement_decision_surface` 或对应轻量原因。
- `planned-dev` 命中触发信号：`deep-plan` 输出“实现前方案审查包”后，路由到 `deep-review` 的 `PRE-IMPLEMENTATION-REVIEW` 模式。
- 审查对象是需求冻结表、规划盘问矩阵、预计变更矩阵、测试设计控制、能力切片、风险反证和开放问题；不是代码 diff。
- 审查发现 P0/P1 时回到 `deep-plan` 修方案；不得直接进入实现。
- L3 或 review pressure 默认使用 `review_mode=codex_only_cross_review`，但必须披露同上下文多镜头不等于外部独立审查。

## 用户可见产物中文优先

生成给用户阅读的 `tech-design.md`、`task_plan.md`、`code-change.md`、审查报告和矩阵时，默认使用中文标题、中文表头和中文说明。英文术语只保留为括号别名或内部状态码，例如“预计变更矩阵（Expected Diff Matrix）”。代码标识、命令、文件名、状态枚举和协议字段保持原文。

## 运行态模型（Runtime State Model）
...
推荐技能链：`pre-flight-check` → `req-alignment-check` → `ideate:planning-brainstorm` → `deep-plan` → `deep-review` 实现前方案审查 → `dev-workflow` → `gen-tests` → `deep-review` 提交前审查 → `sync-progress`。
...
7. 实现前方案审查：L2/L3、review pressure、多 surface 或高返工成本 → `deep-review` 的 `PRE-IMPLEMENTATION-REVIEW`。
8. TDD：行为变更、bugfix、新功能 → `dev-workflow` / `gen-tests`，在对应技能内执行 RED/GREEN 门禁。
9. 实现：方案审查通过 → `dev-workflow`。
10. Bug / Debug：先按证据分型；测试红灯 → `gen-tests` FIX；生产日志/事故，或“生产环境/线上环境 + 业务 selector + 排查现象” → `log-investigator`；无复现/无日志 → 先采证或静态假设，不直接修。跨组件问题先追数据流，三次修复/查询仍无闭环 → `deep-plan`。
11. 收口：实现后 → `gen-tests` → `deep-review` → `sync-progress`。
...
| 有需求文档但方案可能不止一种，或用户要求自主实现/少追问 | `pre-flight-check` → `req-alignment-check` → `ideate:planning-brainstorm` → `deep-plan` → 必要时实现前方案审查 | 规划阶段先替用户交叉追问和审方案，避免实现后靠多轮纠偏 |
| 领域术语混用、同词多义、代码词和业务词冲突 | `pre-flight-check` → `req-alignment-check` 领域语言账本 → `deep-plan` | 先统一 canonical term，避免实现和测试命名漂移 |
| 多入口/接口/页面/导出/任务/日志/展示面 | `pre-flight-check` → `req-alignment-check` | 先建 Surface 矩阵 |
| 跨模块/报表/异步/外部集成/落库 | `pre-flight-check` → `deep-plan` | 先建预计变更矩阵 |
| 跨边界、不可逆、状态/事务、外部协议或高置信但证据不足的决策 | `pre-flight-check` → `deep-plan` 决策反证点，必要时 `deep-review` | 先把 assertion、contract 和反证问题写清，避免自信推进错误方向 |
...
| 给定需求文档后要求自主实现、少问、覆盖 90%+ | `pre-flight-check` → `req-alignment-check` → `ideate:planning-brainstorm` → `deep-plan` → `deep-review` 实现前方案审查 → `dev-workflow` → `gen-tests` → `deep-review` 提交前审查 → `sync-progress` | 作为 `planned-dev:autonomous-delivery` 推进，用覆盖账本、方案审查和核心主链优先级收口；不是 replay/eval |
...
| 生产结论、热修、修数、缓存刷新、外部接口或跨组件归因 | `review_tier=L2`；必要时 `log-investigator` → `req-alignment-check` 同症状分支矩阵 → `deep-plan` / `deep-review` | 防止局部日志或单个修复分支被误报为整体根因闭环 |
...
| 技术方案 | `deep-plan` |
| 从需求到代码 | `pre-flight-check` → `req-alignment-check` → `ideate:planning-brainstorm` → `deep-plan` → 必要时 `deep-review` 实现前方案审查 → `dev-workflow` → `gen-tests` → `deep-review` 提交前审查 → `sync-progress` |
| 从需求文档到 90%+ 自主实现 | `pre-flight-check` → `req-alignment-check` 覆盖账本 → `ideate:planning-brainstorm` → `deep-plan` 90% Coverage Plan → `deep-review` 实现前方案审查 → `dev-workflow` 核心主链优先 → `gen-tests` → `deep-review` 提交前审查 → `sync-progress` |
