...
```json
{
  ...
  "carriers": {
    "ExampleApplyTaskProcessor": {
      "layer": "Task",
      "module": "example-core",
      "file": "example-core/src/main/java/.../ExampleApplyTaskProcessor.java",
      "baseline_commit": "e19c16c",
      "type": "Task"
    }
  }
...
```powershell
$result = .\scripts\Get-CarrierLayer.ps1 -Carrier "ExampleApplyTaskProcessor" -BaselineRoot "<PROJECT_ROOT>"
...
if ($index.carriers.PSObject.Properties.Name -contains "ExampleApplyTaskProcessor") {
    $carrierInfo = $index.carriers."ExampleApplyTaskProcessor"
...
$result = .\scripts\Get-CarrierLayer.ps1 -Carrier "ExampleApplyTaskProcessor"

# Check result
if ($result.layer -eq "Facade" -or $result.layer -eq "Controller") {
    # Valid for core_entry family
    $selected_carrier = "ExampleApplyTaskProcessor"
