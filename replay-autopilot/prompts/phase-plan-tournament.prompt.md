...
```
   - **测试 harness 选择规则（v289/v473/v475/v476）**：`first_red_test` 必须指向已有测试依赖的模块。默认使用当前 worktree 中真实存在的 `<test-module>/src/test/...` + `-pl <test-module> -am` 作为测试 harness；测试目标可以是 `<production-module>` 中的真实 Service/TaskProcessor carrier。`<production-module>` 若无 JUnit/Mockito/Spring Test 依赖，不得规划 `<production-module>/src/test/...` 测试；必须选择一个已证明依赖生产模块的既有 `<test-module>`，并通过依赖调用生产 carrier。所有 Maven RED/GREEN 命令必须包含 `-am`，PowerShell 下带 `-Dtest`/`#` 时使用 `mvn --% ...` 形态。禁止通过修改任何 `pom.xml` 或新增测试依赖来满足 RED。若无法在已有 harness 中证明，写 `PLAN_BLOCKED_TEST_HARNESS` 并把 `plan_status` 降为 `BLOCKED`。
     - **测试模块策略预检（v478/v479）**：在最终确定 plan 前，必须完成并记录 test infrastructure check：
       ...
       7. **Replay-autopilot control-plane 例外（v687）**：当目标 carrier 位于 `replay-autopilot/scripts`、测试 harness 位于 `replay-autopilot/scripts/tests`、且 `first_red_test` 是已有 `Test-v*.ps1` 回归脚本时，`test_infrastructure_check.test_module_for_target` 必须写 `replay-autopilot/scripts/tests`，`compilation_dry_run_command` 必须写可执行的 `powershell`/`pwsh -NoProfile -ExecutionPolicy Bypass -File {{WORKTREE}}\replay-autopilot\scripts\tests\Test-v*.ps1` 命令，并由 runner 写入 `TEST_INFRASTRUCTURE_DRY_RUN.json`。此类控制面 harness 不要求 Maven、`-pl`、`-am`、`test-compile`、模块 `pom.xml` 或 `src/test`；schema gate 会校验 PowerShell 脚本存在、命令指向隔离 worktree、证据 exit_code=0 且输出含 `PASS`/`ALL PASSED`。不得因为 `replay-autopilot/scripts/tests` 没有 POM 而写 `PLAN_BLOCKED_TEST_INFRASTRUCTURE`。
