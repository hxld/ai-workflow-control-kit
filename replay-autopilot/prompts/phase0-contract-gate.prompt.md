...
2. **验证 carrier 匹配**：
   - Calculate Loss task（ExampleCalculatorApiTaskProcessor）仅计算金额，不触发 auto-flow
   - Apply Claim task（ExampleApiTaskProcessor）是综合 AI 接口，在结果保存后触发 auto-flow
...
**关键区分**：
- `ExampleCalculatorApiTaskProcessor`：仅计算损失金额，不是 auto-flow 触发点
- `ExampleApiTaskProcessor`：综合 AI 接口，结果保存后触发 auto-flow，是正确的 auto-flow 入口
