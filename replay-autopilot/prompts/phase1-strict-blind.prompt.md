...
- PowerShell 下不要把 `example-*` 这类裸通配作为路径参数传给 `rg`；用 `rg PATTERN . --glob 'example-core/**'`，或传精确目录列表。
...
- 使用 Maven 必须带 `-f {{WORKTREE}}\pom.xml`；仅当 replay config 定义 `maven_settings` 时才带 `{{MAVEN_SETTINGS_ARG}}`。
