...
function New-PlanFixture {
    ...
    $carrierPath = Join-Path $Root 'worktree\claim-core\src\main\java\com\acme\ExistingCoreFlowService.java'
    Write-Utf8 $carrierPath @'
package com.acme;

public class ExistingCoreFlowService {
    public void process() {
    }
}
'@
...
## V457 Executable Evidence
target_carrier_file_path: claim-core/src/main/java/com/acme/ExistingCoreFlowService.java
target_carrier_line_number: 4
expected_test_class: ExistingCoreFlowServiceTest
expected_test_method: processWritesState
expected_assertions: ["assertEquals(\"DONE\", state.getStatus())", "assertTrue(state.isProcessed())", "assertEquals(1, state.getUpdateCount())"]
expected_side_effects: [{"operation":"UPDATE","target":"state.status","trigger":"ExistingCoreFlowService.process"}]
"@
}
...
$phase1HasHarnessRule = $phase1Prompt.Contains('claim-server/src/test/java/...') -and $phase1Prompt.Contains('claim-core') -and $phase1Prompt.Contains('carrier')
$phase1HasPomRule = $phase1Prompt.Contains('pom.xml') -and $phase1Prompt.Contains('-f {{WORKTREE}}\pom.xml') -and $phase1Prompt.Contains('-pl <test-module> -am')
$planHasHarnessRule = $planPrompt.Contains('PLAN_BLOCKED_TEST_HARNESS') -and $planPrompt.Contains('-pl claim-server -am')
Assert-True $phase1HasHarnessRule 'Phase1 prompt must default to existing claim-server harness while allowing claim-core carriers'
Assert-True $phase1HasPomRule 'Phase1 prompt must forbid POM dependency changes and use isolated worktree Maven command'
