...
Assert-True -Name 'config_push_timeout_present' -Condition ($configText -match '(?m)^knowledge_backup_push_timeout_seconds:\s*\d+\s*$')
...
Assert-True -Name 'runner_push_timeout_guard' -Condition ($runnerText -match 'knowledge_backup_push_timeout_seconds' -and $runnerText -match 'WaitForExit\(\$pushTimeoutSeconds \* 1000\)' -and $runnerText -match 'Stop-Process -Id \$pushProcess\.Id')
Assert-True -Name 'runner_sync_does_not_delegate_push_to_sync_script' -Condition ($runnerText -notmatch "\`$args\s*\+=\s*'-Push'")
Assert-True -Name 'runner_pushes_after_successful_sync' -Condition (
    $runnerText.Contains('Start-Process -FilePath git') -and
    $runnerText.Contains("'-C', `$knowledgeRepo, 'push', 'origin', `$branch")
)
