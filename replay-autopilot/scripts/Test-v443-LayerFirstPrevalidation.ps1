...
function Get-SuggestedFacade {
    ...
    foreach ($pattern in $patterns) {
        ...
        if ($pattern -match "Facade") {
            return @{ Found = $true; ClassName = $pattern; SearchOutput = "example-api/src/main/java/com/example/project/facade/$pattern.java" }
        }
    }
...
Test-Assertion -Name "Facade suggestion class name" -Script {
    $result = Get-SuggestedFacade -ServiceCarrier "ExampleFlowService" -Worktree "D:\worktree"
    $result.ClassName
} -Expected "ExampleAutoClaimFlowFacade"

# Test 10: Facade suggestion for ServiceImpl
Test-Assertion -Name "Facade suggestion for ServiceImpl suffix" -Script {
    $result = Get-SuggestedFacade -ServiceCarrier "ExampleServiceImpl" -Worktree "D:\worktree"
    $result.ClassName
} -Expected "ExampleFacade"
