...
function New-NoChangePolicyFixture {
    ...
    Write-Text (Join-Path $worktree 'claim-core\src\main\java\com\huize\claim\core\ai\task\AiApplyClaimApiTaskProcessor.java') 'class AiApplyClaimApiTaskProcessor { void rebuildTaskData() {} }'
    Write-Text (Join-Path $worktree 'claim-core\src\main\java\com\huize\claim\core\ai\task\AiCalculateLossApiTaskProcessor.java') 'class AiCalculateLossApiTaskProcessor { void rebuildTaskData() {} }'
    ...
    Write-Text (Join-Path $Root 'ORACLE_DIFF_ANALYSIS.json') '{"production_files":2,"high_weight_files":2,"files":[{"path":"claim-core/src/main/java/com/huize/claim/core/ai/task/AiApplyClaimApiTaskProcessor.java","is_production":true,"weight":"HIGH","additions":2},{"path":"claim-core/src/main/java/com/huize/claim/core/ai/task/AiCalculateLossApiTaskProcessor.java","is_production":true,"weight":"HIGH","additions":2}]}'
...
golden_slice_binding: exact_contract_gap -> AiClaimDataAssemblyHelper.RequestBuildFunction -> AiApplyClaimApiTaskProcessor.rebuildTaskData -> req.setPolicyNum(buildContext.getPolicyNum()) -> upstream lambda assignment verified
carrier_search: performed
carrier_search_queries: rg "class.*AiApplyClaimApiTaskProcessor" --type java; rg "class.*AiCalculateLossApiTaskProcessor" --type java; rg "rebuildTaskData" --type java
existing_production_carriers: AiApplyClaimApiTaskProcessor; AiCalculateLossApiTaskProcessor; AiClaimDataAssemblyHelper
selected_carrier_from_search: AiApplyClaimApiTaskProcessor; AiCalculateLossApiTaskProcessor
...
'@
    Write-Text (Join-Path $Root 'PLAN_RESULT.json') '{"plan_status":"PROCEED","target_carrier_file_path":"claim-core/src/main/java/com/huize/claim/core/ai/task/AiApplyClaimApiTaskProcessor.java","highest_weight_open_gate":"wire_payload_api_contract","core_closure_required":false}'
...
| claim-core | core_entry | AiApplyClaimApiTaskProcessor.java | NO_CHANGE | VERIFIED_PRESENT |
...
'@
    ...
    Write-Text (Join-Path $Root 'EXPECTED_DIFF_MATRIX.md') 'AiApplyClaimApiTaskProcessor LOGIC_FIX; AiCalculateLossApiTaskProcessor LOGIC_FIX; -pl claim-server -am'
    ...
    Write-Text (Join-Path $Root 'TEST_CHARTER.md') 'Test harness: No-Spring JUnit with Mockito; claim-server/src/test/java/com/huize/claim/core/ai/task/AiApplyClaimApiTaskProcessorTest.java'
    Write-Text (Join-Path $Root 'FIRST_SLICE_PROOF_PLAN.md') @'
first_slice: S1
first_red_test: claim-server/src/test/java/com/huize/claim/core/ai/task/AiApplyClaimApiTaskProcessorTest.testRebuildTaskData_PolicyNum
