...
selected_carrier_from_search: AiApplyClaimApiTaskProcessor.rebuildTaskData; AiCalculateLossApiTaskProcessor.rebuildTaskData
...
'@
    Write-Text (Join-Path $Root 'PLAN_RESULT.json') '{"plan_status":"PROCEED","target_carrier_file_path":"claim-core/src/main/java/com/huize/claim/core/ai/task/AiApplyClaimApiTaskProcessor.java","highest_weight_open_gate":"core_entry","core_closure_required":true}'
    ...
    Write-Text (Join-Path $Root 'IMPLEMENTATION_CONTRACT.md') 'AiClaimDataAssemblyHelper.buildRequestCommon AiClaimDataAssemblyHelper.RequestBuildFunction RequestBuildContext req.setPolicyNum(buildContext.getPolicyNum()) req.setInsureNum(buildContext.getInsureNum()) shallow GREEN forbidden AiApplyClaimApiTaskProcessor.rebuildTaskData AiCalculateLossApiTaskProcessor.rebuildTaskData'
    Write-Text (Join-Path $Root 'EXPECTED_DIFF_MATRIX.md') 'AiApplyClaimApiTaskProcessor LOGIC_FIX 2 lines; AiCalculateLossApiTaskProcessor LOGIC_FIX 2 lines; -pl claim-server -am'
