...
try {
    ...
    $safeBinding = 'stateful_side_effect -> ExampleDataAssemblyHelper.RequestBuildFunction -> ExampleApplyClaimApiTaskProcessor.rebuildTaskData -> RED: source-chain assignment missing before fix -> GREEN: req.setPolicyNum(buildContext.getPolicyNum()) and req.setInsureNum(buildContext.getInsureNum()) -> executable assertion passes'
    Write-Text (Join-Path $root 'PLAN_RESULT.md') @"
plan_status: PROCEED
carrier_search: performed
carrier_search_queries: rg rebuildTaskData; rg RequestBuildFunction; rg ExampleApplyClaimApiTaskProcessor
existing_production_carriers: example-core/src/main/java/com/example/project/core/ai/task/ExampleApplyClaimApiTaskProcessor.java; example-core/src/main/java/com/example/project/core/ai/task/ExampleCalculatorApiTaskProcessor.java
selected_carrier_from_search: example-core/src/main/java/com/example/project/core/ai/task/ExampleApplyClaimApiTaskProcessor.java
...
first_slice: S1
first_red_test: ExampleApplyClaimApiTaskProcessorTest.testRebuildTaskData_SourceChainAssignment
"@

    Write-Json (Join-Path $root 'PLAN_RESULT.json') ([ordered]@{
        plan_status = 'PROCEED'
        golden_slice_binding = 'stateful_side_effect -> ExampleDataAssemblyHelper.RequestBuildFunction -> ExampleApplyClaimApiTaskProcessor.rebuildTaskData -> RED: request.getPolicyNum() returns null -> GREEN: req.setPolicyNum(buildContext.getPolicyNum()) -> unit test passes'
    ...
    Write-Text (Join-Path $root 'REPLAY_PLAN.md') 'ExampleDataAssemblyHelper.buildRequestCommon and ExampleDataAssemblyHelper.RequestBuildFunction with RequestBuildContext; mvn -pl sample-module -am test-compile'
    Write-Text (Join-Path $root 'IMPLEMENTATION_CONTRACT.md') 'ExampleDataAssemblyHelper.RequestBuildFunction RequestBuildContext req.setPolicyNum(buildContext.getPolicyNum()) req.setInsureNum(buildContext.getInsureNum())'
    Write-Text (Join-Path $root 'EXPECTED_DIFF_MATRIX.md') 'example-core/src/main/java/com/example/project/core/ai/task/ExampleApplyClaimApiTaskProcessor.java LOGIC_FIX'
    ...
    Write-Text (Join-Path $root 'TEST_CHARTER.md') 'example-server/src/test/java sample-module ExampleDataAssemblyHelper.RequestBuildFunction RequestBuildContext'
    Write-Text (Join-Path $root 'FIRST_SLICE_PROOF_PLAN.md') @"
first_slice: S1
first_red_test: ExampleApplyClaimApiTaskProcessorTest.testRebuildTaskData_SourceChainAssignment
golden_slice_binding: $safeBinding
highest_weight_open_gate: core_entry
selected_real_entry: ExampleApplyClaimApiTaskProcessor.rebuildTaskData
selected_carrier: ExampleApplyClaimApiTaskProcessor.rebuildTaskData
target_subsurface_or_carrier: ExampleApplyClaimApiTaskProcessor.rebuildTaskData
...
expected_production_diff: example-core/src/main/java/com/example/project/core/ai/task/ExampleApplyClaimApiTaskProcessor.java
...
forbidden_substitute_proof: uses ExampleDataAssemblyHelper.RequestBuildFunction and RequestBuildContext
...
"@
    ...
    Write-Json (Join-Path $root 'ORACLE_DIFF_ANALYSIS.json') ([ordered]@{
        files = @(
            [ordered]@{ path = 'example-core/src/main/java/com/example/project/core/ai/task/ExampleApplyClaimApiTaskProcessor.java'; is_production = $true; high_weight = $true; additions = '2' }
        )
    })
