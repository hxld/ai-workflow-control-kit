...
try {
    ...
    $binding = 'stateful_side_effect -> AiClaimDataAssemblyHelper.buildRequestCommon -> AiClaimDataAssemblyHelper.RequestBuildFunction -> AiApplyClaimApiTaskProcessor.rebuildTaskData -> RED: source-chain assignment missing before fix -> GREEN: req.setPolicyNum(buildContext.getPolicyNum()) and req.setInsureNum(buildContext.getInsureNum()) -> executable assertion passes'
...
"@
    ...
    Write-Text (Join-Path $replayRoot 'REPLAY_PLAN.md') 'AiClaimDataAssemblyHelper.buildRequestCommon AiClaimDataAssemblyHelper.RequestBuildFunction RequestBuildContext -pl claim-server -am LOGIC_FIX req.setPolicyNum(buildContext.getPolicyNum()) req.setInsureNum(buildContext.getInsureNum()) AiApplyClaimApiTaskProcessor.rebuildTaskData AiCalculateLossApiTaskProcessor.rebuildTaskData'
    Write-Text (Join-Path $replayRoot 'IMPLEMENTATION_CONTRACT.md') 'selected_real_entry: AiApplyClaimApiTaskProcessor.rebuildTaskData, AiCalculateLossApiTaskProcessor.rebuildTaskData AiClaimDataAssemblyHelper.buildRequestCommon AiClaimDataAssemblyHelper.RequestBuildFunction RequestBuildContext req.setPolicyNum(buildContext.getPolicyNum()) req.setInsureNum(buildContext.getInsureNum())'
    Write-Text (Join-Path $replayRoot 'EXPECTED_DIFF_MATRIX.md') 'LOGIC_FIX claim-core/src/main/java/com/huize/claim/core/ai/task/AiApplyClaimApiTaskProcessor.java claim-core/src/main/java/com/huize/claim/core/ai/task/AiCalculateLossApiTaskProcessor.java -pl claim-server -am validation closure status'
...
## GREEN Phase
claim-server unit harness with existing dependencies
'@
...
"@
    Write-Text (Join-Path $replayRoot 'TEST_INFRASTRUCTURE_DRY_RUN.json') '{"command":"mvn -s D:\\maven\\settings\\settings.xml -f worktree\\pom.xml -pl claim-server -am test-compile","exit_code":0,"stdout_tail":"BUILD SUCCESS"}'
    ...
    Write-Host 'PASS: v565 policy rebuild machine harness evidence closes claim-server gate'
} finally {
