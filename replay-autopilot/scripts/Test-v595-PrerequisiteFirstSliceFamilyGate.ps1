...
function New-PlanFixture {
    ...
    Write-Utf8 (Join-Path $worktree 'example-core\src\main\java\com\example\ExampleModuleConfigService.java') @'
package com.example;
class ExampleModuleConfigService {
...
existing_production_carriers: ExampleModuleConfigService; ExampleApplyClaimApiTaskProcessor
...
golden_slice_binding: exact_contract_gap -> config_policy_threshold -> ExampleModuleConfigService -> RED -> GREEN -> DB side effect
first_slice: S1 - config threshold prerequisite
first_red_test: ExampleModuleConfigServiceTest.freeReviewAmount_save_shouldPersist
...
"@
    ...
    Write-Utf8 (Join-Path $Root 'REPLAY_PLAN.md') 'core_entry config_policy_threshold S2 ExampleApplyClaimApiTaskProcessor core tracer'
    Write-Utf8 (Join-Path $Root 'IMPLEMENTATION_CONTRACT.md') 'selected_real_entry: com.example.ExampleApplyClaimApiTaskProcessor.handleTaskResponse()
first_slice: S1 - config threshold prerequisite
first_red_test: ExampleModuleConfigServiceTest.freeReviewAmount_save_shouldPersist
shallow-green-ban: GREEN cannot claim core DONE until S2 core tracer'
    Write-Utf8 (Join-Path $Root 'EXPECTED_DIFF_MATRIX.md') 'validation closure status for ExampleModuleConfigService.java'
    Write-Utf8 (Join-Path $Root 'SIDE_EFFECT_LEDGER.md') 'state task progress log transaction DB insert/update free_review_amount'
    Write-Utf8 (Join-Path $Root 'TEST_CHARTER.md') 'RED GREEN ExampleModuleConfigServiceTest freeReviewAmount_save_shouldPersist'
    Write-Utf8 (Join-Path $Root 'FAMILY_CONTRACT.json') '{"selected_real_entry":"com.example.ExampleApplyClaimApiTaskProcessor.handleTaskResponse()","first_executable_slice":"S1","families":[{"id":"core_entry","required":true,"proof_required":["S2 core tracer"]},{"id":"config_policy_threshold","required":true,"proof_required":["S1 config field"]}]}'
...
selected_real_entry: com.example.ExampleApplyClaimApiTaskProcessor.handleTaskResponse()
public_entry_contract_coverage: not_public_entry_with_reason:first_slice_family_$FirstSliceFamily
selected_carrier: ExampleModuleConfigService
target_subsurface_or_carrier: ExampleModuleConfigService.save
production_boundary: example-core/src/main/java/com/example/ExampleModuleConfigService.java
...
expected_production_diff: example-core/src/main/java/com/example/ExampleModuleConfigService.java
...
target_carrier_file_path: example-core/src/main/java/com/example/ExampleModuleConfigService.java
